...
function sanitizeMojibakeDisplay(input: unknown): string {
...
function toCleanStringArray(input: unknown, limit = 4): string[] {
...
function buildInfoPreviewLines(raw: Record<string, unknown> | null): string[] {
...
function buildTechnicalPreviewLines(raw: Record<string, unknown> | null): string[] {
  const row = toRecord(raw) || {};
...
function hasObjectPayload(value: unknown): value is Record<string, unknown> {
  const row = toRecord(value);
  return Boolean(row && Object.keys(row).length > 0);
}

function pickFirstObject(...candidates: unknown[]): Record<string, unknown> {
...
function summarizeHttpErrorText(input: unknown): string {
...
function resolveBackendBaseUrl(backendBaseUrl?: string): string {
...
function createWorkflowRunId(prefix = "run"): string {
