...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/session/policy/revoke`, {
      method: "POST",
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "proxy_revoke_failed",
        reason: error instanceof Error ? error.message : "proxy_revoke_failed",
      },
      { status: 500 }
    );
  }
