...
const FACTORY_PATH = path.resolve(CONTRACT_DIR, 'KTraceAccountFactory.sol');
...
export function compileKTraceAccountFactory() {
  const input = {
    language: 'Solidity',
    sources: {
      'KTraceAccountFactory.sol': {
        content: fs.readFileSync(FACTORY_PATH, 'utf8')
      }
    },
    settings: {
      optimizer: {
  ...
  const contract = output?.contracts?.['KTraceAccountFactory.sol']?.KTraceAccountFactory;
  if (!contract?.evm?.bytecode?.object) {
    throw new Error('KTraceAccountFactory compilation did not produce bytecode.');
  }

  return {
    sourcePath: FACTORY_PATH,
    contractName: 'KTraceAccountFactory',
    abi: contract.abi || [],
