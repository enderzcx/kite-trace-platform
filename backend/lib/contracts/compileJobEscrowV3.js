...
const ESCROW_PATH = path.resolve(CONTRACT_DIR, 'JobEscrowV3.sol');
...
export function compileJobEscrowV3() {
  const input = {
    language: 'Solidity',
    sources: {
      'JobEscrowV3.sol': {
  ...
  const contract = output?.contracts?.['JobEscrowV3.sol']?.JobEscrowV3;
  if (!contract?.evm?.bytecode?.object) {
    throw new Error('JobEscrowV3 compilation did not produce bytecode.');
  }

  return {
    sourcePath: ESCROW_PATH,
    contractName: 'JobEscrowV3',
    abi: contract.abi || [],
