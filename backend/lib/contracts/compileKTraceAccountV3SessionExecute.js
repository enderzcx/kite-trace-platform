...
const CONTRACT_PATH = path.resolve(CONTRACT_DIR, 'KTraceAccountV3SessionExecute.sol');
...
export function compileKTraceAccountV3SessionExecute() {
  const input = {
    language: 'Solidity',
    sources: {
      'KTraceAccountV3SessionExecute.sol': {
  ...
  const contract =
    output?.contracts?.['KTraceAccountV3SessionExecute.sol']?.KTraceAccountV3SessionExecute;
  if (!contract?.evm?.bytecode?.object) {
    throw new Error('KTraceAccountV3SessionExecute compilation did not produce bytecode.');
  }

  return {
    sourcePath: CONTRACT_PATH,
    contractName: 'KTraceAccountV3SessionExecute',
    abi: contract.abi || [],
