...
4. `npm run verify:mcp:paid` either succeeds or exits with an explicit environment blocker that is documented.
5. `npm run verify:mcp:paid` confirms both `/api/receipt/:requestId` and `/api/evidence/export?traceId=...` after a paid MCP call.
6. `/api/service-invocations` shows `sourceAgentId = mcp-client` for MCP-originated calls.
7. Frontend status card is pointed at the real `.well-known` endpoint only after backend smoke passes.
